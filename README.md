...
現在、Windows x dmd x (dmdに付属の)make の組み合わせでのみ、実行を確認しています。
...
とりあえず、初期状態では amm.exe 実行時のカレントフォルダ名が実行ファイルの名前になってしまいます。
実行ファイル名を root_file.exe とするには、

		>amm root_file.d root_file.exe

とします。

		>amm -version=Unicode -L/exet:nt/su:windows:4.0 root_file.d root_file.exe

'-'(ハイフン)で初まる引数は、dmd へそのまま引き渡されます。

以降で説明する、make-style.xml 内で使用されるマクロを設定するには、

		>amm vwrite=1.0 gui root_file.d

これで、マクロ名 `"vwrite"` に、文字列 `"1.0"` が、マクロ名 `"gui"` に、文字列 `"defined"` が設定されます。
...
- `'target'`
  ...
  コマンドライン引数のうち、拡張子が、`'.exe'`、`'.lib'`、`'.dll'` のものがここに登録される。

  コマンドラインからの指定を省略した場合は、他のマクロの値を参照し、

  `'install_directory'` ~ `'project_directory'` ~ `".exe"` が使われる。
...
- `'res_file'`
  ...
  `'rc_file'` から類推された、コンパイル済リソースファイル名が格納される。
